...
router.get('/', function(req, res) {
  Model.Movie.findAll().then(movies => {
    // res.send(movies)
    res.render('movies/index', {movies:movies, auth:req.session})
  })
})

router.get('/add', auth, function (req,res) {
  Model.Schedule.findAll().then(schedules => {
    res.render('movies/add', {schedules:schedules, auth:req.session})
  })
})
...
router.get('/:id', function (req, res) {
  Model.Movie.findById(req.params.id, {
    ...
    // res.send(movie)
    res.render('movies/detail', {movie:movie, auth:req.session})
  })
})
...
router.get('/edit/:id', auth, function(req,res) {
  Promise.all([
    ...
    res.render('movies/edit', {movie: rows[0], schedules:rows[1], auth:req.session})
  }).catch(error => {
    res.send(error)
  })
})
...
router.get('/book', auth, function(req, res) {
  res.render('movies/book', {auth:req.session})
...
router.get('/:id/book', auth, function(req, res) {
  Promise.all([
    ...
    // res.send(movie)
    res.render('movies/book', {movie:rows[0], profile:rows[1], auth:req.session})
  })
})
